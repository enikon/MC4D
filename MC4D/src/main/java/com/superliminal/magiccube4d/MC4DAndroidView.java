...
public class MC4DAndroidView extends View {
    ...
    private long lastUp0; // When was last time with no finger on screen
    private float[] lastTap0; // Location of the last tap
            ...
            public boolean onTouch(View v, MotionEvent event) {
                ...
                switch(event.getAction() & MotionEvent.ACTION_MASK) {
                    ...
                    case MotionEvent.ACTION_UP: // The last pointer up out of however many.
                        ...
                        if(useAdvancedControl){
                            if (pid == 0 && timeDiff < 500 &&  moveDist < 30) {

                                if(lastTap0 != null) {
                                    timeDiff = now - lastUp0;
                                    Vec_h._VMV2(movement, lastTap0, curPoint);
                                    moveDist = Math.sqrt(Vec_h._NORMSQRD2(movement));

                                    if (timeDiff < 500 &&  moveDist < 30) {
