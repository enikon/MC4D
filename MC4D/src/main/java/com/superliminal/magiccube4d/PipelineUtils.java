...
public class PipelineUtils
{
    ...
    */
    ...
    /**
     * @return true if a given cubie contains a given sticker color; false otherwise.
     */
    public static boolean cubieContainsColor(int cubie, int color, PuzzleDescription puzzle) {
        int sticker2cubie[] = puzzle.getSticker2Cubie();
        int sticker2face[] = puzzle.getSticker2Face();
        for(int s=0; s<puzzle.nStickers(); s++) {
        	if(sticker2cubie[s] == cubie && sticker2face[s] == color)
        		return true;
        }
		return false;
    }
...
//                // Hack: Playing with blindfold ideas.
//                int iFace = sticker2Face[iSticker];
//                if(iFace == 0) // Hide central face.
//                	continue;
//                // Gray out all stickers of all cubies that don't contain the outer face color.
//                boolean gray = !cubieContainsColor(sticker2cubie[iSticker], 7, puzzleDescription);
